...
@section('content')
                    ...
                    <ol class="inline-flex items-center space-x-1 text-sm font-medium md:space-x-2">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <a href="#"
                                    class="ml-1 text-gray-700 hover:text-primary-600 md:ml-2 dark:text-gray-300 dark:hover:text-white">Atribut</a>
                            </div>
                        </li>
                        <li>
                            <div class="flex items-center">
                            ...
                            @foreach ($atribut as $atribute)
                                <tr class="hover:bg-gray-100">
                                    ...
                                    <td class="p-4 space-x-2 whitespace-nowrap">
                                        <button type="button"
                                            data-modal-target="edit-attribute-modal-{{ $atribute->id }}"
                                            data-modal-toggle="edit-attribute-modal-{{ $atribute->id }}"
                                        ...
                                        <button type="button" data-modal-target="delete-user-modal-{{ $atribute->id }}"
                                            data-modal-toggle="delete-user-modal-{{ $atribute->id }}"
    ...
    <div
        ...
        <div class="flex items-center mb-4 sm:mb-0">
            ...
            <span class="text-sm font-normal text-gray-500">Showing <span
                    class="font-semibold text-gray-900">{{ $atribut->firstItem() }}</span> to
                <span class="font-semibold text-gray-900">{{ $atribut->lastItem() }}</span>
                of <span class="font-semibold text-gray-900">{{ $atribut->total() }}</span></span>
        </div>
    ...
    @foreach ($atribut as $atribute)
        <div class="fixed left-0 right-0 z-50 items-center justify-center hidden overflow-x-hidden overflow-y-auto top-4 md:inset-0 h-modal sm:h-full"
            id="edit-attribute-modal-{{ $atribute->id }}">
            <div class="relative w-full h-full max-w-2xl px-4 md:h-auto">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow">
                    <!-- Modal header -->
                    ...
                    <!-- Modal body -->
                    <div class="p-6 space-y-6">
                        <form action="{{ route('atribut.update', $atribute->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="grid grid-cols-6 gap-6">
                                <div class="col-span-6 sm:col-span-3">
                                    ...
                                    <select name="product_id" id="product_id"
                                        ...
                                        @foreach ($products as $product)
                                            <option value="{{ $product->id }}"
                                                {{ old('product_id', $atribute->product_id) == $product->id ? 'selected' : '' }}>
                                                {{ $product->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="col-span-6 sm:col-span-3">
                                    <label for="name"
                                        class="block mb-2 text-sm font-medium text-gray-900">Atribut</label>
                                    <input type="text" name="name" id="name"
                                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5"
                                        placeholder="Masukkan atribut" required
                                        value="{{ old('name', $atribute->name) }}">
                                </div>
                                ...
                                <div class="col-span-6 sm:col-span-3">
                                    ...
                                    <input type="text" name="value" id="value"
                                        class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5"
                                        placeholder="Masukkan nilai" required
                                        value="{{ old('value', $atribute->value) }}">
                                </div>
                            </div>
        ...
        <div class="fixed left-0 right-0 z-50 items-center justify-center hidden overflow-x-hidden overflow-y-auto top-4 md:inset-0 h-modal sm:h-full"
            id="delete-user-modal-{{ $atribute->id }}">
            <div class="relative w-full h-full max-w-md px-4 md:h-auto">
                ...
                <div class="relative bg-white rounded-lg shadow">
                    ...
                    <div class="flex justify-end p-2">
                        <button type="button"
                            class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center"
                            data-modal-hide="delete-user-modal-{{ $atribute->id }}">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                                xmlns="http://www.w3.org/2000/svg">
                                <path fill-rule="evenodd"
                                    d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                    ...
                    <div class="p-6 pt-0 text-center">
                        ...
                        <h3 class="mt-5 mb-6 text-lg text-gray-500">Apakah kamu yakin ingin menghapus atribut?</h3>
                        <form id="delete-form-{{ $atribute->id }}"
                            action="{{ route('atribut.destroy', $atribute->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
